#!/usr/bin/env bash
# One-shot V2V network setup for the HC3 HIL stack.
#
# Usage:
...
# unit that runs tools/hcc_v2v/relay_server.py on 0.0.0.0:19090. Lead joins
# the same hotspot. The PC is not in the V2V path; this matches the real
# two-car field topology and frees the PC's USB link from V2V noise.
#
...
ExecStart=/usr/bin/env python3 $RELAY_PY --host 0.0.0.0 --port 19090
Restart=always
RestartSec=3
