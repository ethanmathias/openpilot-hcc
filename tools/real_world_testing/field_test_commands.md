...
The `Encountered 1 file that should have been a pointer` LFS warning is
harmless — ignore it.
...
1. Plug the ego into the harness (same `hyundai_n` harness as before), power
   the lead over USB. Both boot; with no lab WiFi in range, the ego hosts the
...
Confirm the lead's hotspot IP (expect `10.42.0.60`; substitute below if different):
...
`HYUNDAI_TUCSON_4TH_GEN`, `dashcam= False`, `opLong= True`, and a line
containing `fwdRadar 0x7d0` in the firmware list (that radar is the SCC
hardware the Sportage was missing):
...
re-check. If it still fails, the trim likely lacks SCC — stop and reassess.
Do NOT force the fingerprint.

**Preflight** — all 14 must pass before the car moves; each failure prints
its own fix:
...
The V2V subscriber silently rejects packets whose sender timestamp is more
than 500 ms off the ego's clock. The lead drifts while offline. The fix sets
the lead's clock FROM the ego's (only relative skew matters — the ego is the
reference).

The naive sync always lands ~600 ms behind because of the SSH round-trip, so
pre-compensate by ~0.9 s. Run these two lines together:
...
Tuning: if preflight still shows about −600 ms the offset didn't apply (rerun
both lines). If it shows, say, −150 ms, increase `0.9` to `1.05`; if +300 ms,
decrease to `0.6`. Anything inside ±300 ms passes with margin; last time
`+ 0.9` landed at +228 ms on the first try. A `[WARN]` under 500 ms is OK to
drive on. Re-check before every run — a free-running clock drifts tens of ms
per hour. If the lead ever reboots, its clock resets — re-run this section.
...
- The lead is virtual. NOTHING is physically ahead. In V2V-only mode
  openpilot does NOT brake for real obstacles — the driver is the only
  collision avoidance.
- **CANCEL button = clean disengage.** Brake cuts actuation instantly at the
  panda, but the UI may show engaged for ~2 more seconds — that is expected.
...
1. Hold brake, press SET at standstill, release brake — note whether the car
   holds or creeps (creep is expected; the forced-pid path never enters the
   stopping state).
2. Press CANCEL — confirm clean disengage.
3. Re-engage at ~10 mph, tap the brake — confirm actuation cuts instantly
   (UI disengage may lag ~2 s).
4. Re-engage with the virtual lead publishing, feet off — the moment
   `hccc=` goes nonzero and the car physically responds, the Sportage
   blocker is officially gone.
...
The lead prints `speed scale 0.6197` and `top 20.0 mph` at startup — confirm
you see that line in the streamed output before the profile moves.
`--start_delay 10` holds the initial speed (0 for scn 48) for 10 s: the
driver presses SET during that window, then the profile starts.

The run ends itself at `--duration` and collects automatically. Ctrl-C ends
early and still collects. NEVER pass `--bench` with a real car.

Emergency stop for the data feed (driver's brake is the emergency stop for
the car):
...
Full-speed scenario 48 (32 mph peak) only when you have the road for it:
drop `--max_speed_mph 20`.
...
- `stats.txt` — want ~50 Hz, ~0% loss, max gap < 100 ms.
...
Engaged but no acceleration — check the relay's rejection reasons (look at
the `reason` column of the newest relay CSV):
...
`missing_ego_peer` = the subscriber in controlsd never registered (is
openpilot onroad? is `HCCV2VEnabled=1`?). Timestamp-skew rejections = redo
section 3.
...
Car shows "dashcam mode" mid-session: the car was probably power-cycled
without full ignition — reboot the ego with the car fully on, redo the
fingerprint check in section 2.
